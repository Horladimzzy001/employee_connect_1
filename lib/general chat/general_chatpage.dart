...
Future<void> sendPushNotification(String title, String body, String currentUserPlayerId) async {
  var url = Uri.parse('https://onesignal.com/api/v1/notifications');

  

  // Fetch all player IDs except the current user
  QuerySnapshot usersSnapshot = await FirebaseFirestore.instance.collection('users').get();
  List<String> playerIds = [];

  usersSnapshot.docs.forEach((doc) {
    String? playerId = doc['playerId']; // Fetch the Player ID
    if (playerId != null && playerId != currentUserPlayerId) {
  ...
  if (playerIds.isNotEmpty) {
    var response = await http.post(
      ...
      headers: {
        ...
        "Authorization": "Basic MzQ1ZjU5OTEtYzVhZC00NzE1LThiZGUtNjE1Yzk0MDI1NjI4",
      },
      body: jsonEncode({
...
// In the parent widget or wherever you fetch currentUserData
...
Future<void> _sendMessage() async {
  if (_messageController.text.trim().isNotEmpty || _image != null) {
    ...
    Map<String, dynamic> messageData = {
      ...
      'senderId': widget.currentUser['userId'], // Include senderId here
    ...
    await sendPushNotification(
      ...
      _messageController.text.trim().isNotEmpty ? _messageController.text.trim() : "You have a new image.",
